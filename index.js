function substituiNumerosPares(array) {
	...
	for (let i = 0; i < array.length; i++) {
		if (numPar(array[i]) && naoZero(array[i])) { // Verifica se é par ou 0
			console.log(`trocando ${array[i]} por 0...`);
			array[i] = 0; // Caso seje par troque por 0
		} else if (!naoZero(array[i])) {
			console.log('Ops, você já vale 0!');
		}
	}
	console.log(array); // imprime array modificado
}

substituiNumerosPares([23, 0, 3, 5, 9 , 30, 65, 82, 43, 24,]);